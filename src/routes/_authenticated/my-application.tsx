...
// ─── Workflow stages shown to applicants ───────────────────────────────────
const STAGES = [
...
type Form = {
  ...
  father_name: string; father_phone: string; father_status: string;
  mother_name: string; mother_phone: string; mother_status: string;
...
function MyApplication() {
  ...
  const [f, setF]                     = useState<Form>({ ...EMPTY_FORM, email: user?.email ?? "" });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: app } = await supabase
        .from("applications")
        .select("*, jobs(title, country, employer)")
  ...
  const next = async () => {
    if (step === 0) {
      ...
      if (ninClean.length !== 14) return toast.error(`NIN should be 14 characters (you entered ${ninClean.length})`);
      if (!/^[A-Z0-9]{14}$/i.test(ninClean)) return toast.error("NIN should contain only letters and numbers");
    }
    if (step === 1) {
      if (!f.father_name || !f.father_phone)
        return toast.error("Please enter your father's full name and telephone");
      if (!isValidPhone(f.father_phone))
        return toast.error("Enter a valid telephone number for your father");
      if (!f.mother_name || !f.mother_phone)
        return toast.error("Please enter your mother's full name and telephone");
      if (!isValidPhone(f.mother_phone))
        return toast.error("Enter a valid telephone number for your mother");
      if (!f.next_of_kin_name || !f.next_of_kin_phone || !f.next_of_kin_relationship)
        return toast.error("Please complete next of kin details");
      if (!isValidPhone(f.next_of_kin_phone))
        return toast.error("Enter a valid telephone number for next of kin");
    }
    if (step === 2) {
  ...
  const submit = async () => {
    ...
    if (passportFile) {
      ...
      if (!upErr) {
        ...
        await supabase.from("application_details").update({ passport_photo_path: path } as any).eq("user_id", user.id);
      }
    }

    const fees: any[] = [];
    if (f.has_passport === "no")
      fees.push({ user_id: user.id, amount: FEE_PASSPORT, currency: "UGX", status: "pending", payment_type: "passport_processing", notes: "Passport processing fee." });
    if (f.nin_issue && f.nin_issue !== "no_issues")
      fees.push({ user_id: user.id, amount: FEE_NIN, currency: "UGX", status: "pending", payment_type: "nin_assistance", notes: `NIN assistance: ${NIN_ISSUES.find((x) => x.v === f.nin_issue)?.l ?? f.nin_issue}.` });
    if (fees.length) await supabase.from("payments").insert(fees);

    const { data: existingApp } = await supabase.from("applications").select("id").eq("applicant_id", user.id).limit(1).maybeSingle();
    if (!existingApp) {
      await supabase.from("applications").insert({ applicant_id: user.id, status: "registration_submitted" as any } as any);
    } else {
    ...
    await supabase.from("notifications").insert({
      ...
      message: fees.length
        ...
        : "Your application is received. Our team will review it and contact you shortly.",
    });
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-start justify-between flex-wrap gap-3">
          ...
          <Button variant="outline" size="sm" onClick={() => { setEditing(true); setStep(0); }}>
        ...
        <Card className="p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Badge className="bg-gradient-primary text-primary-foreground">{STAGE_LABEL[status] ?? status.replace(/_/g," ")}</Badge>
          </div>
          ...
          {tracker?.admin_notes && (
            <div className="mt-4 p-3 bg-muted rounded-lg text-sm">
              <span className="font-medium">Admin notes:</span> {tracker.admin_notes}
            </div>
          )}
        </Card>
        ...
        <Card className="p-6 space-y-2">
          ...
          <Review label="Father"            v={`${f.father_name}${f.father_phone ? ` — ${f.father_phone}` : ""}`}/>
          <Review label="Mother"            v={`${f.mother_name}${f.mother_phone ? ` — ${f.mother_phone}` : ""}`}/>
        ...
        {tracker && (tracker.assigned_job_title || tracker.jobs?.title) && (
          <Card className="p-6 border-primary/30">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-bold text-lg">Job assigned by Wakatine</h2>
            </div>
            ...
            {tracker.assigned_job_description && (
              <div className="mt-3 p-3 bg-muted/50 rounded-lg text-sm whitespace-pre-wrap">
                <b>Description:</b> {tracker.assigned_job_description}
              </div>
            )}
          </Card>
        )}
      ...
      <Card className="p-5 md:p-7">
        ...
        {step === 0 && (
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Field label="District of Residence *"><Input placeholder="e.g. Iganga, Kampala" value={f.district} onChange={(e) => upd("district", e.target.value)}/></Field>
            <Field label="Village / Address"><Input value={f.village} onChange={(e) => upd("village", e.target.value)}/></Field>
            <Field label="National ID Number (NIN) *" hint="14 characters, letters and numbers only">
        ...
        {step === 1 && (
          <div className="space-y-6">
            {/* Father */}
            <div>
              <h3 className="font-semibold mb-3 text-primary">Father's Information</h3>
              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="Father's Full Name *">
                  <Input placeholder="e.g. John Mpaye" value={f.father_name} onChange={(e) => upd("father_name", e.target.value)}/>
                </Field>
                <Field label="Father's Telephone *">
                  <Input type="tel" inputMode="tel" placeholder="+256…" value={f.father_phone} onChange={(e) => upd("father_phone", e.target.value)}/>
                </Field>
            ...
            <div className="pt-4 border-t">
              <h3 className="font-semibold mb-3 text-primary">Mother's Information</h3>
              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="Mother's Full Name *">
                  <Input placeholder="e.g. Mary Nakato" value={f.mother_name} onChange={(e) => upd("mother_name", e.target.value)}/>
                </Field>
                <Field label="Mother's Telephone *">
                  <Input type="tel" inputMode="tel" placeholder="+256…" value={f.mother_phone} onChange={(e) => upd("mother_phone", e.target.value)}/>
                </Field>
            ...
            {/* Next of Kin */}
            <div className="pt-4 border-t">
              <h3 className="font-semibold mb-3 text-primary">Next of Kin</h3>
              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="Full Name *">
                  <Input value={f.next_of_kin_name} onChange={(e) => upd("next_of_kin_name", e.target.value)}/>
                </Field>
                <Field label="Telephone *">
                  <Input type="tel" inputMode="tel" placeholder="+256…" value={f.next_of_kin_phone} onChange={(e) => upd("next_of_kin_phone", e.target.value)}/>
                </Field>
                <Field label="Relationship *">
                  <Input placeholder="e.g. Brother, Spouse, Uncle" value={f.next_of_kin_relationship} onChange={(e) => upd("next_of_kin_relationship", e.target.value)}/>
                </Field>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            {f.has_passport === "yes" && (
              <Field label="Passport Number *">
                <Input value={f.passport_number} onChange={(e) => upd("passport_number", e.target.value.toUpperCase())}/>
              </Field>
            )}
            {f.has_passport === "no" && (
              <FeeNotice title="Passport processing required" desc={`Wakatine can process your passport. A fee of UGX ${FEE_PASSPORT.toLocaleString()} will be added after submission.`}/>
            )}
            ...
            <div className="pt-2 border-t">
              ...
              {requiresNinFee && (
                <FeeNotice title="NIN assistance required" desc={`Wakatine can help resolve your NIN issue. A fee of UGX ${FEE_NIN.toLocaleString()} will be added after submission.`}/>
              )}
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            <Field label="Primary desired job *" hint="The single role you most want to do.">
                  ...
                  return (
                    <label key={j} className={`flex items-center gap-2 px-3 py-2 rounded-md border text-sm cursor-pointer ${checked ? "border-primary bg-primary/10" : "border-input hover:bg-muted/50"}`}>
                      <input type="checkbox" checked={checked} onChange={(e) => {
                        const next = e.target.checked ? [...f.preferred_jobs, j] : f.preferred_jobs.filter((x) => x !== j);
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <Review label="Phone"        v={f.phone}/>
            <Review label="NIN"          v={f.nin}/>
            <Review label="District"     v={f.district}/>
            <Review label="Village"      v={f.village}/>
            <Review label="Father"       v={`${f.father_name}${f.father_phone ? ` — ${f.father_phone}` : ""}`}/>
            <Review label="Mother"       v={`${f.mother_name}${f.mother_phone ? ` — ${f.mother_phone}` : ""}`}/>
            <Review label="Next of Kin"  v={`${f.next_of_kin_name} (${f.next_of_kin_relationship}) — ${f.next_of_kin_phone}`}/>
            <Review label="Passport"     v={f.has_passport === "yes" ? `Yes — ${f.passport_number}` : "No (processing required)"}/>
